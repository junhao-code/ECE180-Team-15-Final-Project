...
def calculate_flight_delay_rate_bygroup(data, group_type, airport, orig_data):
    ...
    #tp_name = np.array(tp_name)
    #tp_rate = np.array(tp_rate)
    #tp_name  = list(tp_name[(tp_rate!=0)])
    #tp_rate  = list(tp_rate[(tp_rate!=0)])
...
def plot_pie_condition(type_name, type_rate, year,month, airport):
    ...
    if month>12:
        ...
        plt.savefig('CONDITION_'+str(year)+'_'+airport+'_patch.png', dpi = 300)
    else:
        plt.suptitle(str(year)+'/'+str(month)+' Factors of Flight delay in each CONDITION at '+airport,fontsize=20)
        plt.savefig('CONDITION_'+str(year)+'_'+str(month)+'_'+airport+'_patch.png', dpi = 300)
    plt.close()
    #plt.show()
