...
#a[where_are_inf] = 0
cntc=np.zeros((1,12))
weather=Info['WEATHER_DELAY']
nweather=weather.values
cntw=np.zeros((1,12))
nas=Info['NAS_DELAY']
nnas=nas.values
cntn=np.zeros((1,12))
security=Info['SECURITY_DELAY']
nsecurity=security.values
cnts=np.zeros((1,12))
lateaircraft=Info['LATE_AIRCRAFT_DELAY']
...
for w in range (12):
    ...
    for i in range (nDepDelayATLi.size):
        if nDepDelayATLi[0][i] > 15 and ndayofweek[i]==w+1:
            cntc[0][w] += ncarrier[0][i]
            cntw[0][w] += nweather[0][i]
            cntn[0][w] += nnas[0][i]
            cnts[0][w] += nsecurity[0][i]
            cntl[0][w] += nlateaircraft[0][i]
            #cnt2[0][w]+=1
