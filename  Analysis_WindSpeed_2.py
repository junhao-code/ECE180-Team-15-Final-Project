...
def plot_all_scatter_windspeed(tp_name, tp_rate, year, month, airport):
    if tp_name[-1]=='Calm':
        ...
        tp_rate = tp_rate[0:-1]
    print tp_name, tp_rate
    #marks = [['o','r'],['o','g'],['o','b'],['o','y'],['o','m'],['o','c'],['x','r'],['x','g'],['x','b'],['x','y'],['x','m'],['x','c']]
    if month<=12:
...
def main():
    ...
    for i in range(0,1):
    	...
    	plt.suptitle('%d Flight delay rate in each WINDSPEED at %s'%(y, ap),fontsize=20)
    	plt.savefig('WINDSPEED_'+str(y)+'_'+ap+'_scatter_all.png', dpi = 300)
    	#plt.show()
    # ================= parameters =================
