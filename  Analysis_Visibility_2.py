...
def plot_all_scatter_windspeed(tp_name, tp_rate, year, month, airport):
    if tp_name[-1]=='Calm':
        tp_name = tp_name[0:-1]
        tp_rate = tp_rate[0:-1]
    if month<=12:
    	plt.scatter(tp_name, tp_rate, marker='+', color = 'y' )
    else:
    	plt.scatter(tp_name, tp_rate, 40, marker='o', color = 'r' )

def plot_all_scatter_visibility(tp_name, tp_rate, year, month, airport):
    tp_name = tp_name[0:-1]
    tp_rate = tp_rate[0:-1]
    if month<=12:
        plt.scatter(tp_name, tp_rate, marker='+', color = 'y' )
...
def group_calculation(data, weather_type, year, month, airport):
    ...
    for i in range(group_num):
        ...
        if weather_type=='WINDSPEED':
            plot_all_scatter_windspeed(tp_name, tp_rate, year, month, airport)
        if weather_type=='VISIBILITY':
            plot_all_scatter_visibility(tp_name, tp_rate, year, month, airport)
...
def main():
    ...
    for i in range(0,1):
        ...
        plt.legend(('Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Aug', 'Nov', 'Sep', 'All year'),scatterpoints=1,loc='upper right')
        plt.suptitle('%d Flight delay rate in each VISIBILITY at %s'%(y, ap),fontsize=20)
        plt.savefig('VISIBILITY_'+str(y)+'_'+ap+'_scatter_all.png', dpi = 300)
        #plt.show()
    # ================= parameters =================
